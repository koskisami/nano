...
module.exports = {
	data: new SlashCommandBuilder()
		.setName('mokou')
		.setDescription('Sends a random mokou comic image (made by jokanhiyou) RIP Mokoubot/itdoesnteggs.'),
	async execute(interaction) {
		...
		fs.readdir(path.join(process.cwd(), "assets", "mokou"), (err, files) => {
			try {
				...
				let fullPath = path.join(process.cwd(), "assets", "mokou", file).toString();
